...
public class Logout {
    ...
    public void requestLogout(final SweetAlertDialog sweetAlertDialog, final String headerValue, final LogoutInfoHolder logoutInfoHolder) {
        StringRequest requestToken = new StringRequest(Request.Method.POST,  JSONUrl.BASE_URL+JSONUrl.LOG_OUT_URL, new Response.Listener<String>() {
            @Override
            public void onResponse(String response) {
                try {
                    JSONObject jsonObject=new JSONObject(response);
                    String success=jsonObject.getString("success");
                    logoutInfoHolder.logoutInfo(success,null);
